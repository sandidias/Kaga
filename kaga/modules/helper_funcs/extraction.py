...
def extract_user_and_text(
    ...
    if (
        ...
        if not user_id:
            message.reply_text(
                "Saya tidak memiliki pengguna itu di db saya. Anda akan dapat berinteraksi dengan mereka jika "
...
def extract_unt_fedban(
    ...
    if (
        ...
        if not user_id and not str(user_id).isdigit():
            message.reply_text(
                "Saya tidak memiliki informasi pengguna ini di database saya, jadi Anda tidak akan dapat berinteraksi dengan mereka"
                "Coba balas pesan orang itu atau teruskan pesannya sehingga saya bisa menindaklanjutiny"
            )
            return None, None
